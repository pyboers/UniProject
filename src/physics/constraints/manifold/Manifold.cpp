...
Manifold::Manifold(Element &a, Element &b, vec3 *collisioninfo, int length) {
...
void Manifold::solve(float dt) {
	...
	for (auto &c : points) {
		...
		//tangent1
		{
			...
			float j = ((((1 + (c.a.restitution*c.b.restitution))*tangentVel) + getBiasImpulse(c, dt))*c.divisorT1);
			float tIS = c.tangentImpulseSum1;
			c.tangentImpulseSum1 += j;
			c.tangentImpulseSum1 = c.tangentImpulseSum1 < 0 ? 0 : c.tangentImpulseSum1;
		...
		//tangent2
		{
			...
			float j = ((((1 + (c.a.restitution*c.b.restitution))*tangentVel) + getBiasImpulse(c, dt))*c.divisorT2);
			float tIS = c.tangentImpulseSum2;
			c.tangentImpulseSum2 += j;
			c.tangentImpulseSum2 = c.tangentImpulseSum2 < 0 ? 0 : c.tangentImpulseSum2;
